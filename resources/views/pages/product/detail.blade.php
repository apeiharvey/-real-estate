            ...
            <div class="d-flex align-items-center flex-wrap mr-1">
                ...
                <div class="d-flex align-items-baseline flex-wrap mr-5">
                    ...
                    <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
                        ...
                        <li class="breadcrumb-item text-muted">
                            <a href="{{url()->current()}}" class="text-muted">Detail Produk</a>
                        </li>
                    </ul>
                    <!--end::Breadcrumb-->
                </div>
            ...
            <form id="editForm" onsubmit="return false;">
                <div class="d-flex flex-row">
                    <div class="flex-row-fluid ml-12">
                        ...
                        <div class="card card-custom">  <!-- card-custom -->
                            ...
                            <div class="card-body pt-4">
                                <div class="d-flex p-2 justify-content-center">
                                    ...
                                    @for($i=0;$i<=sizeof(@$selected->images);$i++)
                                    @if(@$selected->images[$i]->type == 'image' && @$selected->images[$i]->is_enabled == true)
                                    <div class="mr-5" id="product_image{{$i}}_wrap">
                                        <span>
                                            <small id="product_toptitle_image{{$i}}">
                                                <b><?php echo @$selected->images[$i]->image_url==@$selected->image?'Gambar utama:':''; ?></b>
                                            </small>
                                        </span><br>
                                        <div    class="image-input image-input-outline" style="background-image: url({{$url['disk']->url(@$selected->images[$i]->image_url)}})"
                                                id="product_image{{$i}}" data-image_id="{{@$selected->images[$i]->id}}">
                                            <a href="{{$url['disk']->url(@$selected->images[$i]->image_url)}}" data-toggle="lightbox" data-gallery="example-gallery">
                                                <div class="image-input-wrapper" style="background-image: url(assets/media/users/100_1.jpg)"></div>
                                            </a>
                                            <label class="btn btn-xs btn-icon btn-circle btn-white btn-hover-text-primary btn-shadow" data-action="change" data-toggle="dropdown" title="" data-original-title="Change avatar">
                                                <i class="ki ki-gear icon-md text-muted"></i>
                                            </label>
                                            <ul class="dropdown-menu dropdown-menu-dark">
                                                <li><a class="dropdown-item" onclick="setToMainImage({{$i}})">Jadikan gambar utama</a></li>
                                            </ul>
                                            <span   class="btn btn-xs btn-icon btn-circle btn-white btn-hover-text-primary btn-shadow" onclick="removeDisplayImage({{$i}})" data-action="remove" data-toggle="tooltip" data-title="Hapus gambar ini. Perubahan akan disimpan setelah Anda klik tombol 'TERBITKAN'">
                                                <i  class="ki ki-bold-close icon-xs text-muted"></i>
                                            </span>
                                        </div>
                                        
                                        <a class="form-text text-muted" id="product_image{{$i}}_info" onclick="removeDisplayImage_resurrect({{$i}})" style="display:none">urungkan ...
                                            <b id="product_image{{$i}}_info_counter"></b>
                                        </a>

                                    </div>
                                    @elseif(@$selected->images[$i]->type == 'video')
                                        <?php $selected->video_link = @$selected->images[$i]->image_url; ?>
                                    @endif
                                    @endfor
                                </div>
                                <div class="d-flex p-2 justify-content-center">
                                    <div class="dropzone dropzone-default dropzone-success" id="dataListUpload">
                                        <div class="dropzone-msg dz-message needsclick">
                                            <h3 class="dropzone-msg-title">Tambah <b>gambar produk</b> dengan taruh disini atau <span class="text-success">cari</span></h3>
                                ...
                                <div class="row mt-10">
                                    <div class="col-lg-6 col-xl-6">
                                        <div class="form-group">
                                            <label>Tipe</label>
                                            <select class="form-control select2 row product_type" id="kt_select2_item_type" name="item_type_id" data-pre="{{@$selected->item_type_id}}" required >
                                                <option label="Label"></option>
                                                @foreach($list_item_type as $item)
                                                    @if($item['value'] == @$selected->item_type_id)
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input class="form-control form-control-lg form-control-solid" type="text" placeholder=""
                                                value="{{@$selected->name}}" maxlength="225"
                                                name="name" required/>
                                        </div>
                                    ...
                                    <div class="col-lg-6 col-xl-6">
                                        <div class="form-group">
                                            ...
                                            <select class="form-control select2" multiple="multiple" id="kt_select2_class" name="class_code[]">
                                                <option label="Label"></option>
                                                <?php
                                                    $list_class_code = explode(",",@$selected->class_code);
                                                ?>
                                                @foreach($list_class_level as $item)
                                                    @if(in_array($item['value'],@$list_class_code))
                                                    <option value="{{$item['value']}}" selected>{{$item['name']}}</option>
                                                    @else
                                                    <option value="{{$item['value']}}">{{$item['name']}}</option>
                                                    @endif
                                        ...
                                        <div class="overlay overlay-block rounded">
                                            <div class="form-group overlay-wrapper p-5">
                                                <label>Kategori</label>
                                                <input type="text" name="selected_category_id" value="{{@$selected->category_id}}" hidden/>
                                                <select class="form-control select2 row select_category" id="kt_select2_category_1" onchange="changeCategory(1)" 
                                                        name="category_id[]" data-level="1" data-allow_engender=0 required>
                                                    <option label="Label"></option>
                                            ...
                                            <div class="overlay-layer rounded bg-primary-o-20" id="kt_select2_category_loading">
                                ...
                                <div class="row mt-10">
                                    <div class="col-lg-6 col-xl-6">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input class="form-control form-control-lg form-control-solid" type="text" placeholder="contoh: https://www.youtube.com/embed/2gThxAqoUJU"
                                                name="video_link" value="{{@$selected->video_link}}" onkeypress="return event.charCode != 32"/>
                                            <div class="align-items-end"><br>
                                                <a style="{{@$selected->video_link?'':'display:none'}}" id="preview-video" href="{{@$selected->video_link?$selected->video_link:''}}" data-toggle="lightbox" data-gallery="mixedgallery" class="btn btn-light-youtube btn-shadow font-weight-bold mr-2">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <textarea   class="form-control form-control-lg form-control-solid" placeholder="" 
                                                        name="description" required rows="5">{{@$selected->description}}</textarea>
                                        </div>
                                            ...
                                            <div class="col-lg-7 col-xl-7">
                                                <div class="radio-inline mt-3">
                                                    <label class="radio radio-primary">
                                                        <input type="radio" name="is_packet" value=1 required {{@$selected->packet_description != ''?'checked':''}}>
                                                        <span></span>Ya
                                                    </label>
                                                    <label class="radio radio-primary">
                                                        <input type="radio" name="is_packet" value=0 required {{@$selected->packet_description == ''?'checked':''}}>
                                                        <span></span>Tidak
                                                    </label>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="form-group packet_description_wrap" style="{{@$selected->packet_description == ''?'display:none':''}}">
                                            <label>Deskripsi Paket</label>
                                            <textarea   class="form-control form-control-lg form-control-solid"
                                                        name="packet_description" {{@$selected->packet_description == ''?'':'required'}} rows="5">{{@$selected->packet_description}}</textarea>
                                        </div>
                                    </div>
                                    <div class="col-lg-6 col-xl-6">
                                    </div>
                        ...
                        <div class="card card-custom overlay overlay-block" id="kt_modal_price_loading_wrap">
                            <div class="card-header py-3">
                                <div class="card-title align-items-start flex-column">
                            ...
                            <div class="card-body">
                                <div class="overlay-wrapper">
                                        ...
                                        <div class="col-lg-5">
                                            <div class="form-group">
                                                ...
                                                <div class="input-group">
                                                    ...
                                                    <input  class="form-control form-control-lg form-control-solid currency_mask" type="text" name="listing_price" 
                                                            value="{{@$selected->listing_price}}" required/>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <label class="checkbox checkbox-rounded mb-5">
                                        <input type="checkbox" name="is_every_zone_same_price" value=1 {{@$selected->is_every_zone_same_price == 1?'checked':''}}>
                                    ...
                                    <div class="card-scrollable-x">
                                        ...
                                        <div class="bg-white rounded shadow-sm card-scrollable-x-item {{$is_every_zone_same_price__affected_div_class}}">
                                            <center>
                                                <span id="pricelist_zone{{$i}}_title" class="label label-warning label-pill label-inline mr-2 font-size-h5">ZONA {{$i}}</span><br><br>
                                                    ...
                                                    <div class="row">
                                                        <div class="col-lg-11">
                                                            <div class="input-group">
                                                                ...
                                                                <input  class="form-control form-control-lg form-control-solid is_every_zone_same_price__affected_input_class currency_mask" type="text" 
                                                                        value="{{@$price_map['RETAIL'][$i]['price'][0]}}"
                                                                        name="price[retail][{{$i}}][price]" required/>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                ...
                                <div class="overlay-layer bg-dark-o-10" id="kt_modal_price_loading">
                                    <div class="spinner spinner-primary"></div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            <div class="col-lg-6 col-xl-6">
                                <div class="card card-custom">
                                    <div class="card-header py-3">
                                        <div class="card-title align-items-start flex-column">
                                            <h3 class="card-label font-weight-bolder text-dark">Stok</h3>
                                    ...
                                    <div class="card-body">
                                        <div class="form-group row">
                                            <label class="col-xl-3 col-lg-3 col-form-label">Stok Terakhir</label>
                                            <div class="col-lg-9 col-xl-9">
                                                    ...
                                                    <div class="col-lg-7">
                                                        ...
                                                        <select class="form-control select2 row" id="kt_select2_uom" onchange="changeUoM()"
                                                            ...
                                                            @foreach($list_uom as $item)
                                                                @if($item['value'] == @$selected->sales_uom)
                                                                <option value="{{$item['value']}}" selected>{{$item['name']}}</option>
                                                                @else
                                                                <option value="{{$item['value']}}">{{$item['name']}}</option>
                                                                @endif
                                                ...
                                                <div class="row mt-5">
                                                    <div class="col-12">
                                                        <!--begin::MAINTAIN_STOCK -->
                                                        <div class="accordion accordion-solid accordion-panel accordion-svg-toggle" id="accordionStock">
                                                            <div class="card bg-primary">
                                                                <div class="card-header" id="headingStock">
                                                                    <div class="card-title" data-toggle="collapse" data-target="#collapseStock">
                                                                        <div class="card-label">Atur Stok</div>
                                                                        <span class="svg-icon">
                                                                ...
                                                                <div id="collapseStock" class="collapse show" data-parent="#accordionStock">
                                                                    <div class="card-body">
                                                                    
                                                                            <div class="row mt-5">
                                                                                <div class="col-lg-7">
                                                                                    <label class="form-control-label" style="color:white" title="aksi terhadap stock">Aksi</label>
                                                                                    <select class="form-control row" id="kt_select2_stock_op" name="stock_nature">
                                                                                        <option value="+">Tambah (+)</option>
                                                                                        <option value="-">Kurang (-)</option>
                                                                                    </select>
                                                                                </div>
                                                                                <div class="col-lg-5">
                                                                                    <label class="form-control-label" style="color:white">Jumlah</label>
                                                                                    <input  type="number" class="form-control form-control-lg form-control-solid" min="1"
                                                                                            name="stock_amount">
                                                                                </div>
                                                                            </div>
                                                                            <div class="row mt-5 pr-5">
                                                                                <button type="button" class="btn btn-danger btn-lg btn-block button-form-action button-adjust-stock"
                                                                                        data-id="{{@$selected->id}}">
                                                                                    <i class="fas fa-cog fa-sm mr-3"></i>
                                                                                    <span id="button-adjust-stock-info">Konfirmasi Perubahan</span><br>
                                                                                </button>
                                                                            </div>
                                        ...
                                        <div class="row mt-10">
                                            <div class="col-12">
                                                <label class="checkbox checkbox-rounded">
                                                    <input type="checkbox" name="is_free_ongkir" {{@$selected->is_free_ongkir == 1?'checked':''}}>
                            ...
                            <div class="card-body">
                                    ...
                                    <div class="col-lg-9 col-xl-9">
                                        <div class="row">
                                            <div class="col-lg-4">
                                                ...
                                                <div class="input-group">
                                                    <input  type="number" class="form-control form-control-lg form-control-solid" min="0"
                                                            value="{{@$selected->dimension_length}}"
                                            ...
                                            <div class="col-lg-4">
                                                ...
                                                <div class="input-group">
                                                    <input  type="number" class="form-control form-control-lg form-control-solid" min="0"
                                                            value="{{@$selected->dimension_width}}" 
                                            ...
                                            <div class="col-lg-4">
                                                ...
                                                <div class="input-group">
                                                    <input  type="number" class="form-control form-control-lg form-control-solid" min="0"
                                                            value="{{@$selected->dimension_height}}" 
                                    ...
                                    <div class="col-lg-9 col-xl-9">
                                        <div class="radio-inline mt-3">
                                            @foreach($list_item_condition as $item)
                                                <label class="radio radio-primary">
                                                    @if($item['value'] == @$selected->item_condition_type_id)
                                                    <input type="radio" name="item_condition_type_id" value="{{$item['value']}}" required checked>
                                                    @else
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-lg-9 col-xl-9">
                                        <span class="switch switch-sm switch-icon">
                                            <label>
                                                <input type="checkbox" class="product_origin" name="is_umkm" value=true {{@$selected->is_umkm == true?'checked':''}}><span></span>
                                            </label>
                                        </span>
                                    </div>
                                </div>
                                <div class="form-group row">
                                    ...
                                    <div class="col-lg-9 col-xl-9">
                                        <span class="switch switch-sm switch-icon">
                                            <label>
                                                <input type="checkbox" class="product_origin" name="made_in_indonesia" value=false {{@$selected->made_in_indonesia == false?'checked':''}}><span></span>
                                            </label>
                                        </span>
                                    </div>
                                </div>
                                <!-- <div class="form-group row">
                                    ...
                                    <div class="col-lg-9 col-xl-9">
                                        <span class="switch switch-sm switch-icon">
                                            <label>
                                                <input type="checkbox" class="product_origin" name="is_kemendikbud" value=1 {{-- @$selected->is_kemendikbud == 1?'checked':'' --}}><span></span>
                                            </label>
                                        </span>
                                    </div>
                                </div> -->
                                        ...
                                        <div class="row">
                                            <div class="col-lg-4">
                                                <input class="form-control form-control-lg form-control-solid" type="text" placeholder="" maxlength="10"
                                                    name="supplier_code" value="{{@$selected->supplier_code}}"/>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <!--begin::PUBLISH -->
                        <button type="button" class="btn btn-danger btn-lg btn-block button-form-action button-edit pb-5 pt-5" id="btn-form-submit" data-id="{{@$selected->id}}">
                            <i class="fas fa-paper-plane fa-sm mr-3"></i>
                            <span id="button-edit-info">Terbitkan</span><br>
                            <span>(perubahan ini akan menunggu persetujuan Admin lagi)</span>
                        </button>
                        <!--end::PUBLISH -->
                    </div>
                </div>
...
@section('admin_js')
    ...
    <script src="{{ asset('js/pages/product/form_update.js') }}?ver=00011"></script>
@endsection
