      ...
      <form method="post" action="{{route('product.update',$product->id)}}">
        ...
        <div class="form-group">
          ...
          <select name="size[]" class="form-control selectpicker"  multiple data-live-search="true">
              ...
              <option value="ALL" @if( in_array( "ALL",$data ) ) selected @endif>All Size</option>
              @endforeach
          </select>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
          <label for="status" class="col-form-label">Button Link <span class="text-danger">*</span></label> <button type="button" id="btn-add-link" class="btn btn-sm btn-primary">Add Link</button>
            @if(!empty($product->links))
            <div class="link-container" data-count="{{count($product->links)}}">
              @for($i = 0; $i < count($product->links); $i++)
                @include('backend.product.row_link')
              @endfor
            </div>
            @else
            <div class="link-container" data-count=""></div>
            @endif
...
<script>
    ...
    $(document).ready(function() {
      ...
      $('#btn-add-link').on('click',function(e){
        e.preventDefault();
        var count = $('.link-container').data('count');
        $.ajax({
          url:"{{url('/admin/product/get-row-link')}}",
          type:"POST",
          data:{
              _token:"{{csrf_token()}}",
              count:count,
          ...
          success:function(view){
            $('.link-container').append(view);
            $('.link-container').data('count',++count);
          }
        })
      })

      $(document).on('click','.btn-del-link',function(e){
        e.preventDefault();
  ...
  var  child_cat_id='{{$product->child_cat_id}}';
        ...
        $('#cat_id').change(function(){
            ...
            if(cat_id !=null){
                // ajax call
                $.ajax({
                    url:base_url+"/admin/category/"+cat_id+"/child",
                    type:"POST",
                    data:{
                        _token:"{{csrf_token()}}"
                    },
